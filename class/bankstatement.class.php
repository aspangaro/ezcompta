 ...
 * Copyright (C) 2026		Florian Henry				<florian.henry@scopen.fr>
 *
...
class BankStatement extends CommonObject
{
	 ...
	 */
	public $table_element = 'bank_import';

	/**
	 * @var string 	If permission must be checkec with hasRight('ezcompta', 'read') and not hasright('mymodyle', 'bankstatement', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'ezcompta';

	/**
	 * @var string 	String with name of icon for bankstatement. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'bankstatement@ezcompta' if picto is file 'img/object_bankstatement.png'.
	 */
	...
	public $fields = array(
		...
		"label_origin" => array("type" => "varchar(255)", "label" => "Labelorigin", "enabled" => "1", 'position' => 35, 'notnull' => 1, "visible" => "-1",),
		"comment" => array("type" => "text", "label" => "Comment", "enabled" => "1", 'position' => 40, 'notnull' => 0, "visible" => "-1",),
		"note" => array("type" => "text", "label" => "Note", "enabled" => "1", 'position' => 45, 'notnull' => 0, "visible" => "-1",),
		"bdate" => array("type" => "date", "label" => "Bdate", "enabled" => "1", 'position' => 50, 'notnull' => 0, "visible" => "-1",),
		"vdate" => array("type" => "date", "label" => "Vdate", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
		"date_scraped" => array("type" => "datetime", "label" => "Datescraped", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-1",),
		"original_amount" => array("type" => "double(24,8)", "label" => "Originalamount", "enabled" => "1", 'position' => 65, 'notnull' => 0, "visible" => "-1",),
		"original_currency" => array("type" => "varchar(255)", "label" => "Originalcurrency", "enabled" => "1", 'position' => 70, 'notnull' => 0, "visible" => "-1",),
		"amount_debit" => array("type" => "double(24,8)", "label" => "Amountdebit", "enabled" => "1", 'position' => 75, 'notnull' => 1, "visible" => "-1",),
		...
		"fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserModif", "enabled" => "1", 'position' => 115, 'notnull' => -1, "visible" => "-2", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
		"import_key" => array("type" => "varchar(14)", "label" => "ImportId", "enabled" => "1", 'position' => 900, 'notnull' => 0, "visible" => "-2",),
		"datas" => array("type" => "text", "label" => "Datas", "enabled" => "1", 'position' => 125, 'notnull' => 1, "visible" => "-1",),
	);
	public $rowid;
	public $id_account;
	public $record_type;
	public $label;
	public $record_type_origin;
	public $label_origin;
	public $comment;
	public $note;
	public $bdate;
	...
	//  */
	// public $table_element_line = 'ezcompta_bankstatementline';
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'BankStatement', 'fk_element'=>'fk_bankstatement'));
	...
	//  */
	// protected $childtablesoncascade = array('ezcompta_bankstatementdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('ezcompta', 'bankstatement', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ezcompta', 'bankstatement', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ezcompta', 'bankstatement_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'bankstatement/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'bankstatement/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->ezcompta->dir_output.'/bankstatement/'.$oldref;
				$dirdest = $conf->ezcompta->dir_output.'/bankstatement/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->ezcompta->dir_output.'/bankstatement/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ezcompta','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('ezcompta','ezcompta_advance','validate'))))
		 {
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowBankStatement")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("BankStatement").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/ezcompta/bankstatement_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_bankstatement:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('EZCOMPTA_BANKSTATEMENT_ADDON')) {
			...
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/ezcompta/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefBankStatement $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/ezcompta/doc/";
...
class BankStatementLine extends CommonObjectLine
{
	 ...
	 */
	public $parent_element = '';		// Example: '' or 'bankstatement'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_bankstatement'
